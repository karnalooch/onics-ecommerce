...
Compatible with: Claude Code, Codex, OpenCode, Gemini CLI, Antigravity.
...
Agents are loaded automatically from `.claude/agents/` (or `.agent/rules/`).
Invoke via the subagent tool or reference in prompt:
```
...
# AI Toolkit — Configuration Reference
...
- **lint**: Lint code with auto-detected tools.
- **mcp-builder**: Build production-grade MCP servers.
...
- **verification-before-completion**: Mandatory verification before claiming work is done.

## Quality Standards (Constitution)

**Article I — Safety First**
- No data loss: never delete files without backup verification.
- No blind execution: never run LLM-generated code without review.
- No infinite loops: all autonomous loops must have a max iteration (max 3).
